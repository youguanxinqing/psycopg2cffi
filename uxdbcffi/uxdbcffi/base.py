...
PostgreSQL's own ``ux_get_constraintdef()`` builtin procedure.  This function
...
illustrates this behavior::
    ...
    test=> SELECT ux_catalog.ux_get_constraintdef(r.oid, true) FROM
    test-> ux_catalog.ux_class c JOIN ux_catalog.ux_namespace n
    test-> ON n.oid = c.relnamespace
    test-> JOIN ux_catalog.ux_constraint r  ON c.oid = r.conrelid
...
PG ``search_path`` and then asked ``ux_get_constraintdef()`` for the
...
The same query against ``ux_get_constraintdef()`` now returns the fully
...
SQLAlchemy will by default use the return value of ``ux_get_constraintdef()``
...
class ENUM(sqltypes.NativeForEmulated, sqltypes.Enum):
    ...
    def _check_for_name_in_memos(self, checkfirst, kw):
        ...
        if "_ddl_runner" in kw:
            ...
            if "_ux_enums" in ddl_runner.memo:
                ux_enums = ddl_runner.memo["_ux_enums"]
            else:
                ux_enums = ddl_runner.memo["_ux_enums"] = set()
            present = (self.schema, self.name) in ux_enums
            ux_enums.add((self.schema, self.name))
            return present
        else:
            return False
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def post_create_table(self, table):
        table_opts = []
        ux_opts = table.dialect_options["postgresql"]

        inherits = ux_opts.get("inherits")
        ...
        if ux_opts["partition_by"]:
            table_opts.append("\n PARTITION BY %s" % ux_opts["partition_by"])

        if ux_opts["with_oids"] is True:
            table_opts.append("\n WITH OIDS")
        elif ux_opts["with_oids"] is False:
            table_opts.append("\n WITHOUT OIDS")

        if ux_opts["on_commit"]:
            on_commit_options = ux_opts["on_commit"].replace("_", " ").upper()
            table_opts.append("\n ON COMMIT %s" % on_commit_options)

        if ux_opts["tablespace"]:
            tablespace_name = ux_opts["tablespace"]
            table_opts.append(
...
class PGDialect(default.DefaultDialect):
    ...
    def do_recover_twophase(self, connection):
        resultset = connection.execute(
            sql.text("SELECT gid FROM ux_prepared_xacts")
        )
    ...
    def has_schema(self, connection, schema):
        query = (
            "select nspname from ux_namespace " "where lower(nspname)=:schema"
        )
    ...
    def has_table(self, connection, table_name, schema=None):
        # seems like case gets folded in ux_class...
        if schema is None:
            cursor = connection.execute(
                sql.text(
                    "select relname from ux_class c join ux_namespace n on "
                    "n.oid=c.relnamespace where "
                    "ux_catalog.ux_table_is_visible(c.oid) "
                    "and relname=:name"
                ).bindparams(
                    sql.bindparam(
                        "name",
    ...
    def has_type(self, connection, type_name, schema=None):
        if schema is not None:
            query = """
            SELECT EXISTS (
                SELECT * FROM ux_catalog.ux_type t, ux_catalog.ux_namespace n
            ...
            query = """
            SELECT EXISTS (
                SELECT * FROM ux_catalog.ux_type t
                WHERE t.typname = :typname
                AND ux_type_is_visible(t.oid)
                )
                """
            query = sql.text(query)
    ...
    def get_table_oid(self, connection, table_name, schema=None, **kw):
        ...
        if schema is not None:
            ...
            schema_where_clause = "ux_catalog.ux_table_is_visible(c.oid)"
        query = (
            """
            SELECT c.oid
    ...
    def get_schema_names(self, connection, **kw):
        result = connection.execute(
            sql.text(
                "SELECT nspname FROM ux_namespace "
                "WHERE nspname NOT LIKE 'ux_%' "
                "ORDER BY nspname"
    ...
    def get_table_names(self, connection, schema=None, **kw):
        result = connection.execute(
            sql.text(
                "SELECT c.relname FROM ux_class c "
                "JOIN ux_namespace n ON n.oid = c.relnamespace "
    ...
    def get_view_definition(self, connection, view_name, schema=None, **kw):
        view_def = connection.scalar(
            sql.text(
                "SELECT ux_get_viewdef(c.oid) view_def FROM ux_class c "
                "JOIN ux_namespace n ON n.oid = c.relnamespace "
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        SQL_COLS = """
            SELECT a.attname,
              ux_catalog.format_type(a.atttypid, a.atttypmod),
              (SELECT ux_catalog.ux_get_expr(d.adbin, d.adrelid)
                FROM ux_catalog.ux_attrdef d
               WHERE d.adrelid = a.attrelid AND d.adnum = a.attnum
               AND a.atthasdef)
              AS DEFAULT,
              a.attnotnull, a.attnum, a.attrelid as table_oid,
              pgd.description as comment
            FROM ux_catalog.ux_attribute a
            LEFT JOIN ux_catalog.ux_description pgd ON (
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        if self.server_version_info < (8, 4):
            PK_SQL = """
                ...
                FROM
                    ux_class t
                    join ux_index ix on t.oid = ix.indrelid
                    join ux_attribute a
        ...
        PK_CONS_SQL = """
        SELECT conname
           FROM  ux_catalog.ux_constraint r
    ...
    def get_foreign_keys(
        ...
        **kw
        ...
        FK_SQL = """
          SELECT r.conname,
                ux_catalog.ux_get_constraintdef(r.oid, true) as condef,
        ...
        for conname, condef, conschema in c.fetchall():
            ...
            if postgresql_ignore_search_path:
                ...
                # ux_get_constraintdef().  If the schema is in the search
                # path, ux_get_constraintdef() will give us None.
    ...
    def _ux_index_any(self, col, compare_to):
    ...
    def get_indexes(self, connection, table_name, schema, **kw):
        ...
        if self.server_version_info < (8, 5):
            IDX_SQL = """
              ...
              FROM
                  ux_class t
                        join ux_index ix on t.oid = ix.indrelid
                        join ux_class i on i.oid = ix.indexrelid
                        left outer join
                            ux_attribute a
                            on t.oid = a.attrelid and %s
                        left outer join
            ...
            """ % (
                ...
                self._ux_index_any("a.attnum", "ix.indkey"),
            )
        else:
            IDX_SQL = """
              SELECT
        ...
        for row in c.fetchall():
            ...
            if not has_idx:
                ...
                # (new in pg 8.3)
                # "ux_index.indoption" is list of ints, one per column/expr.
    ...
    def get_check_constraints(self, connection, table_name, schema=None, **kw):
        ...
        CHECK_SQL = """
            SELECT
                cons.conname as name,
                ux_get_constraintdef(cons.oid) as src
            FROM
                ux_catalog.ux_constraint cons
            WHERE
                cons.conrelid = :table_oid AND
    ...
    def _load_enums(self, connection, schema=None):
        ...
        SQL_ENUMS = """
            SELECT t.typname as "name",
               ...
               ux_catalog.ux_type_is_visible(t.oid) as "visible",
               n.nspname as "schema",
               e.enumlabel as "label"
            FROM ux_catalog.ux_type t
                 LEFT JOIN ux_catalog.ux_namespace n ON n.oid = t.typnamespace
                 LEFT JOIN ux_catalog.ux_enum e ON t.oid = e.enumtypid
            WHERE t.typtype = 'e'
        """
    ...
    def _load_domains(self, connection):
        ...
        SQL_DOMAINS = """
            SELECT t.typname as "name",
               ux_catalog.format_type(t.typbasetype, t.typtypmod) as "attype",
