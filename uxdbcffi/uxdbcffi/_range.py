...
from uxdbcffi._impl.exceptions import ProgrammingError, InterfaceError
from uxdbcffi.extensions import ISQLQuote, adapt, register_adapter, b
from uxdbcffi.extensions import new_type, new_array_type, register_type
...
def register_range(pgrange, pyrange, conn_or_curs, globally=False):
    ...
    *pgrange* type and raises `~uxdb.ProgrammingError` if the type is not
...
class RangeCaster(object):
    ...
    def _from_db(self, name, pyrange, conn_or_curs):
        ...
        """
        from uxdbcffi.extensions import STATUS_IN_TRANSACTION
        from uxdbcffi.extras import _solve_conn_curs

        conn, curs = _solve_conn_curs(conn_or_curs)

        if conn.server_version < 90200:
            raise ProgrammingError(
                "range types not available in version %s" % conn.server_version
            )
