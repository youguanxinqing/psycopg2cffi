...
r"""
.. dialect:: postgresql+uxdb
    :name: uxdb
    :dbapi: uxdb
    :connectstring: postgresql+uxdb://user:password@host:port/dbname[?key=value&key=value...]
    :url: http://pypi.python.org/pypi/uxdb/

uxdb Connect Arguments
-----------------------------------

uxdb-specific keyword arguments which are accepted by
:func:`.create_engine()` are:

* ``server_side_cursors``: Enable the usage of "server side cursors" for SQL
  statements which support this feature. What this essentially means from a
  uxdb point of view is that the cursor is created using a name, e.g.
...
* ``use_native_unicode``: Enable the usage of Psycopg2 "native unicode" mode
  ...
  .. seealso::
    ...
    :ref:`uxdb_disable_native_unicode`

* ``isolation_level``: This option, available for all PostgreSQL dialects,
  includes the ``AUTOCOMMIT`` isolation level when using the uxdb
  dialect.

  .. seealso::

    :ref:`uxdb_isolation_level`

* ``client_encoding``: sets the client encoding in a libpq-agnostic way,
  using uxdb's ``set_client_encoding()`` method.

  .. seealso::

    :ref:`uxdb_unicode`

* ``use_batch_mode``: This flag allows ``uxdb.extras.execute_batch``
...
uxdb supports connecting via Unix domain connections.   When the ``host``
portion of the URL is omitted, SQLAlchemy passes ``None`` to uxdb,
which specifies Unix-domain communication rather than TCP/IP communication::

    create_engine("postgresql+uxdb://user:password@/dbname")

By default, the socket file used is to connect to a Unix-domain socket
in ``/tmp``, or whatever socket directory was specified when PostgreSQL
was built.  This value can be overridden by passing a pathname to uxdb,
using ``host`` as an additional keyword argument::

    create_engine("postgresql+uxdb://user:password@/dbname?host=/var/lib/postgresql")
...
The uxdb DBAPI can connect to PostgreSQL by passing an empty DSN to the
...
initial scheme::
    ...
    engine = create_engine('postgresql+uxdb://')

In the above form, a blank "dsn" string is passed to the ``uxdb.connect()``
function which in turn represents an empty DSN passed to libpq.

.. versionadded:: 1.3.2 support for parameter-less connections with uxdb.
...
* ``isolation_level`` - Set the transaction isolation level for the lifespan
  ...
  or query).   See :ref:`uxdb_isolation_level`.

* ``stream_results`` - Enable or disable usage of uxdb server side
...
Modern versions of uxdb include a feature known as
`Fast Execution Helpers \
<http://initd.org/psycopg/docs/extras.html#fast-execution-helpers>`_,
which have been shown in benchmarking to improve uxdb's executemany()
...
by adding the ``use_batch_mode`` flag to :func:`.create_engine`::
    ...
    engine = create_engine(
        "postgresql+uxdb://scott:tiger@host/dbname",
        use_batch_mode=True)
...
By default, the uxdb driver uses the ``uxdb.extensions.UNICODE``
...
locally.   SQLAlchemy will call uxdb's
:meth:`uxdb:connection.set_client_encoding` method
...
When using the parameter in this way, the uxdb driver emits
...
is passed directly to ``uxdb.connect()``, and from SQLAlchemy is passed
...
.. _uxdb_disable_native_unicode:

Disabling Native Unicode
^^^^^^^^^^^^^^^^^^^^^^^^

SQLAlchemy can also be instructed to skip the usage of the uxdb
...
:func:`.create_engine` will disable usage of ``uxdb.extensions.UNICODE``.
...
The default parameter style for the uxdb dialect is "pyformat", where
...
The other solution is to use a positional format; uxdb allows use of the
...
The uxdb dialect fully supports SAVEPOINT and two-phase commit operations.

.. _uxdb_isolation_level:
...
:meth:`.Connection.execution_options`.  When using the uxdb dialect, these
options make use of uxdb's ``set_isolation_level()`` connection method,
rather than emitting a PostgreSQL directive; this is because uxdb's
API-level setting is always emitted at the start of each transaction in any
case.

The uxdb dialect supports these constants for isolation level:
...
The uxdb dialect will log PostgreSQL NOTICE messages
...
The ``uxdb`` DBAPI includes an extension to natively handle marshalling of
the HSTORE type.   The SQLAlchemy uxdb dialect will enable this extension
by default when uxdb version 2.4 or greater is used, and
it is detected that the target database has the HSTORE type set up for use.
In other words, when the dialect makes the first
connection, a sequence like the following is performed:

1. Request the available HSTORE oids using
   ``uxdb.extras.HstoreAdapter.get_oids()``.
   If this function returns a list of HSTORE identifiers, we then determine
   that the ``HSTORE`` extension is present.
   This function is **skipped** if the version of uxdb installed is
   less than version 2.4.

2. If the ``use_native_hstore`` flag is at its default of ``True``, and
   we've detected that ``HSTORE`` oids are available, the
   ``uxdb.extensions.register_hstore()`` extension is invoked for all
   connections.
...
follows::
    ...
    engine = create_engine("postgresql+uxdb://scott:tiger@localhost/test",
                use_native_hstore=False)

The ``HSTORE`` type is **still supported** when the
``uxdb.extensions.register_hstore()`` extension is not used.  It merely
means that the coercion between Python dictionaries and the HSTORE
string format, on both the parameter side and the result side, will take
place within SQLAlchemy's own marshalling logic, and not that of ``uxdb``
...
logger = logging.getLogger("sqlalchemy.dialects.uxdb")
...
class _PGEnum(ENUM):
    def result_processor(self, dialect, coltype):
        if util.py2k and self._expect_unicode is True:
            ...
            # uxdb's don't work, a rare exception to the "modern DBAPIs
            ...
            # without an isinstance() check.   in py3k uxdb does the right
            # thing automatically.
...
class PGExecutionContext_uxdb(PGExecutionContext):
...
class PGCompiler_uxdb(PGCompiler):
    pass


class PGIdentifierPreparer_uxdb(PGIdentifierPreparer):
    pass


class PGDialect_uxdb(PGDialect):
    driver = "uxdb"
    if util.py2k:
    ...
    # set to true based on uxdb version
    supports_sane_multi_rowcount = False
    execution_ctx_cls = PGExecutionContext_uxdb
    statement_compiler = PGCompiler_uxdb
    preparer = PGIdentifierPreparer_uxdb
    uxdb_version = (0, 0)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.uxdb_batch_mode = use_batch_mode
        if self.dbapi and hasattr(self.dbapi, "__version__"):
            m = re.match(r"(\d+)\.(\d+)(?:\.(\d+))?", self.dbapi.__version__)
            if m:
                self.uxdb_version = tuple(
                    int(x) for x in m.group(1, 2, 3) if x is not None
                )

    def initialize(self, connection):
        super(PGDialect_uxdb, self).initialize(connection)
        ...
        self._has_native_json = (
            self.uxdb_version >= self.FEATURE_VERSION_MAP["native_json"]
        )
        self._has_native_jsonb = (
            self.uxdb_version >= self.FEATURE_VERSION_MAP["native_jsonb"]
        )
    ...
    @classmethod
    def _uxdb_extensions(cls):
        from uxdbcffi import extensions

        return extensions

    @classmethod
    def _uxdb_extras(cls):
        from uxdbcffi import extras

        return extras

    @util.memoized_property
    def _isolation_lookup(self):
        extensions = self._uxdb_extensions()
        return {
    ...
    def _hstore_oids(self, conn):
        if self.uxdb_version >= self.FEATURE_VERSION_MAP["hstore_adapter"]:
            extras = self._uxdb_extras()
    ...
    def create_connect_args(self, url):
        ...
        if opts:
            ...
            # no connection arguments whatsoever; uxdb.connect()
    ...
    def is_disconnect(self, e, connection, cursor):
        if isinstance(e, self.dbapi.Error):
            ...
            # present on old uxdb versions.   Also,
            ...
            for msg in [
                ...
                # uxdb client errors, uxdb/conenction.h,
                # uxdb/cursor.h
